...
RESPONSE_NEGATIVE_JSON = js.loads(
...
RESPONSE_NEUTRAL_JSON = js.loads(
...
def test_sentiment_analyzer(mock_post):
    """
    Test sentiment_analyzer function.
    """
    ...
    assert "label" in sentiment_dict
    assert "score" in sentiment_dict
...
def test_sentiment_analyzer_bad_status_code(mock_post):
    ...
    mock_post.side_effect = mock_post_func
    try:
        _ = sentiment_analyzer("I love this new technology.")
        assert False
